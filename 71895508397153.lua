...
local AnimationDB = {
    ...
    Harken = {
         Old = {
            Stunned = "78891328741658",
            CalmDown = "131499371929545",
            Enrage = "120866104804758",
            Immolate = "132318932241644",
            Throw = "126580988391757",
            TanglePull = "87263246883235",
         },
          New = {
             Inmolate = "132318932241644",
             Thow = "126580988391757",
             Tanglpull = "87263246883235",
     Pursuer = {
           Old = {
              Cleave = "87295326479150",
          },
            New = {
              Cleave = "87295326479150",
           }         
...
local function getRoleInfo()
    local ga = Workspace:FindFirstChild("GameAssets")
    if not ga then return "Unknown", nil end

    local teams = ga:FindFirstChild("Teams")
    if not teams then return "Unknown", nil end

    if teams:FindFirstChild("Killer") and teams.Killer:FindFirstChild(lp.Name) then
        local model = teams.Killer[lp.Name]
        return "Killer", model:GetAttribute("KillerName")
    end

    if teams:FindFirstChild("Survivor") and teams.Survivor:FindFirstChild(lp.Name) then
        return "Survivor", nil
    end
...
local function getAbilitiesFolder()
    ...
    for _, teamName in ipairs({ "Survivor", "Killer" }) do
        local team = teams:FindFirstChild(teamName)
        if team then
            local model = team:FindFirstChild(lp.Name)
            if model and model:FindFirstChild("Animations") then
                return model.Animations:FindFirstChild("Abilities") or model.Animations
            end
        end
    end
...
local selectedStyle = "Old" -- "Old" or "New"

local function applyAnimations()
    local role, killerName = getRoleInfo()
    ...
    if role == "Survivor" then
        animSet = AnimationDB.Survivor and AnimationDB.Survivor[selectedStyle]
    elseif role == "Killer" and killerName then
        animSet = AnimationDB[killerName] and AnimationDB[killerName][selectedStyle]
    end

    if not animSet then
        warn("[Animation] No animation set for:", role, killerName)
        return
    end

    for animName, id in pairs(animSet) do
        -- IMPORTANT: skip empty IDs
        if id ~= "" then
...
tabOther:CreateButton({
    ...
    Callback = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/G7bQ6kqx/raw", true))()
        end)
        if not success then
...
tabOther:CreateButton({
    ...
    Callback = function()
        local success, err = pcall(function()
            loadstring(game:HttpGet("https://pastefy.app/DHM3NfNb/raw", true))()
        end)
        if not success then
