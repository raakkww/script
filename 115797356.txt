...
local function IsVisible(targetPosition)
    ...
    if rayResult then
        ...
        if hitCharacter then
            ...
            if hitPlayer and hitPlayer ~= LocalPlayer and not IsTeammate(hitPlayer) then return true end
        end
        return false
    end
    return true
...
-- ==================== CHAMS ====================
function ApplyCham(character, state, color, transparency)
...
TitleText.Text = "Cb Ro | Private Cheat [DEL]"
...
-- VISUALS
CreateToggle("Enable ESP", tabContents["Visuals"], false, function(val) VisualsSettings.ESPEnabled = val; if not val then for _,b in pairs(PlayerBoxes) do b:Remove() end; table.clear(PlayerBoxes) end end)
CreateToggle("Enable Chams", tabContents["Visuals"], false, function(val)
    VisualsSettings.ChamsEnabled = val
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ApplyCham(p.Character, val, VisualsSettings.ChamColor, VisualsSettings.ChamsTransparency)
        end
    end
end)
CreateColorPicker("Cham Color", tabContents["Visuals"], Color3.fromRGB(255,0,0), function(col)
    ...
    if VisualsSettings.ChamsEnabled then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ApplyCham(p.Character, true, col, VisualsSettings.ChamsTransparency)
            end
        end
    end
end)
CreateSlider("Chams Transparency", tabContents["Visuals"], 0, 1, 0.3, function(val)
    ...
    if VisualsSettings.ChamsEnabled then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                ApplyCham(p.Character, true, VisualsSettings.ChamColor, val)
            end
        end
    end
end)
...
-- ==================== ОТКРЫТИЕ/ЗАКРЫТИЕ МЕНЮ НА DELETE ====================
    ...
    if VisualsSettings.ESPEnabled then
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Head") then
                ...
                if root then
                    local hp, onScreen = Camera:WorldToViewportPoint((head.CFrame * CFrame.new(0, 0.5, 0)).Position)
                    if onScreen then
                        local dist = (head.Position - Camera.CFrame.Position).Magnitude
                        local scale = math.clamp(2500 / dist, 8, 350)
                        box.Visible = true
                        box.Size = Vector2.new(scale / 2, scale)
                        box.Position = Vector2.new(hp.X - scale/4, hp.Y - scale/2)
                        box.Color = IsTeammate(player) and Color3.fromRGB(0, 200, 0) or Color3.fromRGB(255, 40, 40)
                    else
                        box.Visible = false
                    end
                end
    ...
    if AimbotSettings.Enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head") then
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Head") then
                ...
                if not AimbotSettings.Wallbang and not IsVisible(player.Character.Head.Position) then continue end
                ...
                if onScreen then
                    ...
                    if dist < closestDist then
                        closestDist = dist
                        bestTarget = player.Character.Head
                    end
                end
            end
        end
...
local function OnCharacterAdded(character)
    ...
    if VisualsSettings.ESPEnabled and player ~= LocalPlayer then
        wait(0.1)
